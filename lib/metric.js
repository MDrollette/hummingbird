...
Metric.prototype = {
  ...
  getPastData: function(num, time_unit, callback) {
    ...
    cursor.toArray(function(err,docs) {
      ...
      //add metric data from db
      for(var i = 0; i< docs.length; i++) {
        totals = self.fromDb(docs[i],totals);
      }
      
      //add metric data waiting to be written to the db
      totals = self.fromDb(self.queued_data,totals); 
  ...
  resetData: function() {
    if(this.fromDb) {
      this.queued_data = this.fromDb(this.data, this.queued_data);
    }
  ...
  resetMinuteData: function() {
    this.queued_data = JSON.parse(JSON.stringify(this.initialData));
